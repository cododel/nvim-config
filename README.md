...
Внутри дерева `h` сворачивает текущую директорию, `l` раскрывает выбранную директорию только на один уровень и повторно не сворачивает её; на файле `l` открывает preview, как стандартный `Tab`. `Enter` делает директорию новым root, а выбранный файл открывает в editor.
...
`Cmd+P` открывает Telescope в центре экрана и сразу запускает поиск по именам файлов. `Cmd+Shift+F` открывает тот же popup сразу в режиме поиска по содержимому через `rg`. Внутри popup доступны режимы:
